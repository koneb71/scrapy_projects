...
class spider1(scrapy.Spider):
    ...
    def start_requests(self):
        for url in self.start_urls:
            ...
            df = pd.read_csv('/Users/neiellcare/Documents/workplace/scraping_hub/sinklaw_sites.csv')
            df2 = df.drop_duplicates(subset='link').reset_index(drop=True)
            df3 = df2.dropna(subset=['link'])
            for keyword, search_operator, link in df3.itertuples(index=False):
    ...
    def parse3(self, response):
        ...
        item['email'] = re.findall('([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)', str(response.body))
